...
public class OpensearchConsolePersistence implements ConsolePersistence {
    ...
    public void save(final Console console) {
        try {
            connection.getClient()
                    .index(new IndexRequest(INDEX).id(console.getId())
    ...
    public List<Console> get() {
        ...
        try {
            response = connection.getClient()
                    .search(new SearchRequest(INDEX).source(
    ...
    public void saveV2(ConsoleV2 console,
        ...
        try {
            connection.getClient()
                    .index(new IndexRequest(INDEX_V2).id(console.getId())
    ...
    public List<ConsoleV2> getV2() {
        try {
            SearchResponse response = connection.getClient()
                    .search(new SearchRequest(INDEX_V2).source(
                                    new SearchSourceBuilder().query(boolQuery().must(matchAllQuery()))
                                            .size(SCROLL_SIZE)
    ...
    private void saveOldConsole(ConsoleV2 console) {
        ...
        try {
            connection.getClient()
                    .index(new IndexRequest(INDEX_HISTORY).id(id)
                            .source(mapper.writeValueAsBytes(console), XContentType.JSON)
                            .setRefreshPolicy(WriteRequest.RefreshPolicy.IMMEDIATE), RequestOptions.DEFAULT);
