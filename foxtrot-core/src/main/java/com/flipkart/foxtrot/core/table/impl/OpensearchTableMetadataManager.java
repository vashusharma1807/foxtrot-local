...
@Singleton
@Order(15)
public class OpensearchTableMetadataManager implements TableMetadataManager {

    public static final String CARDINALITY_CACHE_INDEX = "table_cardinality_cache";
    private static final Logger logger = LoggerFactory.getLogger(OpensearchTableMetadataManager.class);
    ...
    @Inject
    public OpensearchTableMetadataManager(HazelcastConnection hazelcastConnection,
    ...
    private static <K, V> Collector<Entry<K, V>, ?, List<Map<K, V>>> mapSize(int limit) {
    ...
    private MapConfig tableMapConfig() {
        ...
        mapStoreConfig.setFactoryImplementation(TableOpensearchMapStore.factory(opensearchConnection));
    ...
    private TableFieldMapping getTableFieldMapping(String table) {
        ...
        List<FieldMetadata> fieldMetadata = indicesName.stream()
                ...
                .map(index -> mappingsResponse.mappings()
                        .get(index)
                        .get(OpensearchUtils.DOCUMENT_TYPE_NAME))
                .flatMap(mappingData -> {
                    try {
        ...
        for (Map<String, FieldMetadata> innerMap : listOfMaps) {
            ...
            innerMap.values()
                    .forEach(fieldMetadata -> {
                        ...
                        SearchRequest query = new SearchRequest(index).indicesOptions(
            ...
            public Void visit(CardinalityEstimationData cardinalityEstimationData) {
                if (cardinalityEstimationData.getCount() > 0 && cardinalityEstimationData.getCardinality() > 0) {
                    ...
                    if (cardinalityEstimationData.getCardinality() <= 100 || (countToCardinalityRatio > 100
                        ...
                        multiQuery.add(new SearchRequest(index).indicesOptions(IndicesOptions.lenientExpandOpen())
    ...
    private void saveCardinalityCache(String table,
        ...
        try {
            opensearchConnection.getClient()
                    .index(new IndexRequest(CARDINALITY_CACHE_INDEX, OpensearchUtils.DOCUMENT_TYPE_NAME, table).timeout(
    ...
    private List<TableFieldMapping> getAllCardinalityCache() {
        ...
        try {
            SearchResponse response = opensearchConnection.getClient()
                    .search(new SearchRequest(CARDINALITY_CACHE_INDEX).types(OpensearchUtils.DOCUMENT_TYPE_NAME)
